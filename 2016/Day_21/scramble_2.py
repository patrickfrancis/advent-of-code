...
def un_rotate_on_position(word: str, letter: str) -> str:
    idx = word.find(letter)
    if idx % 2 == 1:
        return rotate_left(word, 1 + (idx - 1)//2)
    else:
        return rotate_left(word, 2 + (((idx-2)%8) + 8)//2)
...
if __name__ == "__main__":
    ...
    for inst in reversed(moves):
        ...
        m = rp_pat.match(inst)
        if m:
            scramble = un_rotate_on_position(scramble, m.group(1))
            continue
        ...
        if m:
            scramble = move(scramble, int(m.group(2)), int(m.group(1)))
            continue
    print("unscrambled password: {0}".format(scramble))
